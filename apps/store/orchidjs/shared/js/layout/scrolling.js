!(function (exports) {
  ...
  const Scrolling = {
    ...
    handlePointerMove: function (event) {
      ...
      if (this.isWobbly) {
        ...
        for (let index = 0; index < children.length; index++) {
          ...
          child.style.translate = `${-this.scrollX}px ${-this.scrollY}px`;
          if (this.heldElement === child) {
            child.style.transition = '';
          } else {
            child.style.transition = `translate ${0.5 * distance}s cubic-bezier(0.1, 0, 0, 1.1)`;
          }
        }
      } else {
    ...
    scroll: function name(x, y) {
      ...
      this.scrollElement.style.translate = `${-this.scrollX}px ${-this.scrollY}px`;
